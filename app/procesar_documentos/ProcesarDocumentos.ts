...
export const ProcesarDocuemntos = (data: ProcesarArchivoType, ruc: string, sucursal: number) => {
    ...
    cabecera.map(documento => {
        ...
        const documentoEstructurados = CrearEstructuraCabecera(documento, ruc, sucursal);
        const index = respuestaSunat.findIndex(documentoDeclarado =>
            documentoDeclarado.DOCUMENTO == `${documentoEstructurados.CodVenta}-${documentoEstructurados.TipoDoc}` &&
        ...
        const cuotasEstructuradas = CrearEstructuraCuotas(cuotas, documento.CORRELATIV);
